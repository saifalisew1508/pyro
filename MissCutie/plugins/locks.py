...
@Bot.on_message(command("locktypes"))
...
@Bot.on_message(command("lock") & restrict_filter)
async def lock_perm(c: Bot, m: Message):
...
@Bot.on_message(command("locks") & restrict_filter)
...
@Bot.on_message(command("unlock") & restrict_filter)
async def unlock_perm(c: Bot, m: Message):
    if len(m.text.split()) < 2:
