...
@Bot.on_message(command("approve") & admin_filter)
async def approve_user(c: Bot, m: Message):
...
async def disapprove_user(c: Bot, m: Message):
    db = Approve(m.chat.id)
...
@Bot.on_message(command("approved") & admin_filter)
...
@Bot.on_message(command("approval") & filters.group)
async def check_approval(c: Bot, m: Message):
    db = Approve(m.chat.id)
...
@Bot.on_callback_query(filters.regex("^unapprove_all$"))
