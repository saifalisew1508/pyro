...
from MissCutie import LOGGER, OWNER_ID, SUPPORT_GROUP, SUPPORT_STAFF
from MissCutie.bot_class import Bot
from MissCutie.tr_engine import tlang
from MissCutie.utils.caching import ADMIN_CACHE, admin_cache_reload
from MissCutie.utils.custom_filters import command, restrict_filter
from MissCutie.utils.extract_user import extract_user
from MissCutie.utils.parser import mention_html
from MissCutie.utils.string import extract_time
from MissCutie.vars import Config


@Bot.on_message(command("tban") & restrict_filter)
async def tban_usr(c: Bot, m: Message):
...
@Bot.on_message(command("stban") & restrict_filter)
async def stban_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.ban.no_target"))
        await m.stop_propagation()
...
@Bot.on_message(command("dtban") & restrict_filter)
async def dtban_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.ban.no_target"))
        await m.stop_propagation()
...
@Bot.on_message(command("kick") & restrict_filter)
async def kick_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@Bot.on_message(command("skick") & restrict_filter)
async def skick_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.kick.no_target"))
        return
...
@Bot.on_message(command("dkick") & restrict_filter)
async def dkick_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.kick.no_target"))
        return
    if not m.reply_to_message:
...
@Bot.on_message(command("unban") & restrict_filter)
async def unban_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@Bot.on_message(command("sban") & restrict_filter)
async def sban_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.ban.no_target"))
        await m.stop_propagation()
...
@Bot.on_message(command("dban") & restrict_filter)
async def dban_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.ban.no_target"))
        await m.stop_propagation()
...
@Bot.on_message(command("ban") & restrict_filter)
async def ban_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.ban.no_target"))
        await m.stop_propagation()
...
@Bot.on_callback_query(regex("^unban_"))
async def unbanbutton(c: Bot, q: CallbackQuery):
