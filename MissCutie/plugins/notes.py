...
from MissCutie.utils.cmd_senders import send_cmd
from MissCutie.utils.custom_filters import admin_filter, command, owner_filter
from MissCutie.utils.kbhelpers import ikb
from MissCutie.utils.msg_types import Types, get_note_type
...
@Bot.on_message(command("save") & admin_filter & ~filters.bot)
...
async def get_note_func(c: Bot, m: Message, note_name, priv_notes_status):
...
async def get_raw_note(c: Bot, m: Message, note: str):
...
@Bot.on_message(filters.regex(r"^#[^\s]+") & filters.group & ~filters.bot)
async def hash_get(c: Bot, m: Message):
...
@Bot.on_message(command("get") & filters.group & ~filters.bot)
async def get_note(c: Bot, m: Message):
...
@Bot.on_message(command(["privnotes", "privatenotes"]) & admin_filter & ~filters.bot)
...
@Bot.on_message(command("notes") & filters.group & ~filters.bot)
...
@Bot.on_message(command("clear") & admin_filter & ~filters.bot)
...
@Bot.on_message(command("clearall") & owner_filter & ~filters.bot)
...
@Bot.on_callback_query(filters.regex("^clear_notes$"))
