...
from MissCutie import LOGGER, SUPPORT_GROUP
...
from MissCutie.utils.string import build_keyboard, parse_button


@Bot.on_message(command("pin") & admin_filter)
...
@Bot.on_message(command("unpin") & admin_filter)
async def unpin_message(c: Bot, m: Message):
    try:
        if m.reply_to_message:
...
@Bot.on_message(command("unpinall") & admin_filter)
...
@Bot.on_callback_query(regex("^unpin_all_in_this_chat$"))
async def unpinall_calllback(c: Bot, q: CallbackQuery):
...
@Bot.on_message(command("antichannelpin") & admin_filter)
...
@Bot.on_message(command("pinned") & admin_filter)
async def pinned_message(c: Bot, m: Message):
...
@Bot.on_message(command("cleanlinked") & admin_filter)
...
@Bot.on_message(command("permapin") & admin_filter)
