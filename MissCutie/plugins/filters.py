...
from MissCutie.bot_class import LOGGER, Bot
...
from MissCutie.utils.msg_types import Types, get_filter_type
...
@Bot.on_message(command("filters") & filters.group & ~filters.bot)
...
@Bot.on_message(command(["filter", "addfilter"]) & admin_filter & ~filters.bot)
...
@Bot.on_message(command(["stop", "unfilter"]) & admin_filter & ~filters.bot)
...
@Bot.on_callback_query(filters.regex("^rm_allfilters$"))
...
async def send_filter_reply(c: Bot, m: Message, trigger: str):
...
@Bot.on_message(filters.text & filters.group & ~filters.bot, group=69)
async def filters_watcher(c: Bot, m: Message):
    chat_filters = db.get_all_filters(m.chat.id)
