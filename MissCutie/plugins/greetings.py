...
from MissCutie.utils.custom_filters import admin_filter, bot_admin_filter, command
from MissCutie.utils.msg_types import Types, get_wlcm_type
from MissCutie.utils.parser import escape_markdown, mention_html
from MissCutie.utils.string import (
    build_keyboard,
...
@Bot.on_message(command("cleanwelcome") & admin_filter)
...
@Bot.on_message(command("cleangoodbye") & admin_filter)
...
@Bot.on_message(command("cleanservice") & admin_filter)
...
@Bot.on_message(command("setwelcome") & admin_filter & bot_admin_filter)
...
@Bot.on_message(command("setgoodbye") & admin_filter & bot_admin_filter)
...
@Bot.on_message(command("resetgoodbye") & admin_filter & bot_admin_filter)
...
@Bot.on_message(command("resetwelcome") & admin_filter & bot_admin_filter)
...
@Bot.on_message(filters.service & filters.group, group=59)
...
@Bot.on_chat_member_updated(filters.group, group=69)
async def member_has_joined(c: Bot, member: ChatMemberUpdated):
    if (
...
@Bot.on_chat_member_updated(filters.group, group=99)
async def member_has_left(c: Bot, member: ChatMemberUpdated):
    if (
...
@Bot.on_message(command("welcome") & admin_filter)
async def welcome(c: Bot, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_welcome_status()
    oo = db.get_welcome_text()
    args = m.text.split(" ", 1)
...
@Bot.on_message(command("goodbye") & admin_filter)
async def goodbye(c: Bot, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_goodbye_status()
    oo = db.get_goodbye_text()
    args = m.text.split(" ", 1)
