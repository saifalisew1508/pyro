...
from MissCutie import LOGGER, MESSAGE_DUMP, SUPPORT_STAFF
from MissCutie.bot_class import Bot
from MissCutie.database.antispam_db import ANTISPAM_BANNED, GBan
from MissCutie.database.approve_db import Approve
from MissCutie.database.blacklist_db import Blacklist
from MissCutie.database.group_blacklist import BLACKLIST_CHATS
...
from MissCutie.utils.regex_utils import regex_searcher

# Initialise
gban_db = GBan()


@Bot.on_message(filters.linked_channel)
async def antichanpin_cleanlinked(c: Bot, m: Message):
...
@Bot.on_message(filters.text & filters.group, group=5)
...
@Bot.on_message(filters.user(list(ANTISPAM_BANNED)) & filters.group)
async def gban_watcher(c: Bot, m: Message):
    from MissCutie import SUPPORT_GROUP
...
@Bot.on_message(filters.chat(BLACKLIST_CHATS) & filters.new_chat_members)
async def bl_chats_watcher(c: Bot, m: Message):
    from MissCutie import SUPPORT_GROUP
