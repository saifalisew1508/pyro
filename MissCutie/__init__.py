...
try:
    if environ.get("ENV"):
        ...
        from MissCutie.vars import Development as Config
...
LOGGER.info("------------------------")
LOGGER.info("|      MissCutie_Bot     |")
LOGGER.info("------------------------")
LOGGER.info(f"Owner: {Config.OWNER_ID}")
LOGGER.info("Source Code: https://github.com/saifalisew1508/MissCutie_Bot\n")
...
async def load_cmds(all_plugins):
    ...
    for single in all_plugins:
        ...
        imported_module = imp_mod(f"MissCutie.plugins.{single}")
