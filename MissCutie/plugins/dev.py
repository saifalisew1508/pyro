...
from MissCutie import LOGFILE, LOGGER, MESSAGE_DUMP, UPTIME
from MissCutie.bot_class import Bot
from MissCutie.database.chats_db import Chats
from MissCutie.tr_engine import tlang
from MissCutie.utils.clean_file import remove_markdown_and_html
from MissCutie.utils.custom_filters import command
from MissCutie.utils.http_helper import HTTPx
from MissCutie.utils.parser import mention_markdown
from MissCutie.vars import Config


@Bot.on_message(command("ping", sudo_cmd=True))
...
@Bot.on_message(command("logs", dev_cmd=True))
async def send_log(c: Bot, m: Message):
...
@Bot.on_message(command("ginfo", sudo_cmd=True))
async def group_info(c: Bot, m: Message):
...
@Bot.on_message(command("speedtest", dev_cmd=True))
async def test_speed(c: Bot, m: Message):
    await c.send_message(
        MESSAGE_DUMP,
        f"#SPEEDTEST\n\n**User:** {(await mention_markdown(m.from_user.first_name, m.from_user.id))}",
    )
...
@Bot.on_message(command("neofetch", dev_cmd=True))
...
@Bot.on_message(command(["eval", "py"], dev_cmd=True))
async def evaluate_code(c: Bot, m: Message):
...
@Bot.on_message(command(["exec", "sh"], dev_cmd=True))
...
@Bot.on_message(command("ip", dev_cmd=True))
async def public_ip(c: Bot, m: Message):
...
@Bot.on_message(command("chatlist", dev_cmd=True))
async def chats(c: Bot, m: Message):
...
@Bot.on_message(command("uptime", dev_cmd=True))
...
@Bot.on_message(command("leavechat", dev_cmd=True))
async def leave_chat(c: Bot, m: Message):
    if len(m.text.split()) != 2:
...
@Bot.on_message(command("chatbroadcast", dev_cmd=True))
async def chat_broadcast(c: Bot, m: Message):
    if m.reply_to_message:
