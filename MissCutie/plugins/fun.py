...
@Bot.on_message(command("slap"))
async def fun_slap(c: Bot, m: Message):
    me = await c.get_me()
    ...
    if user_id == me.id:
        temp = choice(fun_strings.SLAP_TEMPLATE)
    else:
...
@Bot.on_message(command("bluetext"))
...
@Bot.on_message(command("weebify"))
