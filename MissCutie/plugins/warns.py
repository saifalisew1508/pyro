...
from MissCutie import LOGGER, SUPPORT_STAFF
...
from MissCutie.utils.custom_filters import admin_filter, command, restrict_filter
...
async def warn(c: Bot, m: Message):
...
@Bot.on_message(command("resetwarns") & restrict_filter)
async def reset_warn(c: Bot, m: Message):
    if len(m.command) <= 1 and not m.reply_to_message:
        await m.reply_text("I can't warn nothing! Tell me user whom I should warn")
        return
...
@Bot.on_message(command("warns") & filters.group)
async def list_warns(c: Bot, m: Message):
    user_id, user_first_name, _ = await extract_user(c, m)
...
async def remove_warn(c: Bot, m: Message):
    if len(m.command) <= 1 and not m.reply_to_message:
        await m.reply_text(
...
async def remove_last_warn_btn(c: Bot, q: CallbackQuery):
    try:
...
@Bot.on_message(command(["warnings", "warnsettings"]) & admin_filter)
...
@Bot.on_message(command("warnmode") & admin_filter)
...
@Bot.on_message(command("warnlimit") & admin_filter)
