...
from MissCutie.utils.http_helper import HTTPx, http
...
async def gdpr_remove(_, m: Message):
    ...
    await m.reply_text(
        ...
        "Note that this will not unban you from any chats, as that is telegram data, not Bot data."
...
async def id_info(c: Bot, m: Message):
...
async def my_info(c: Bot, m: Message):
    try:
