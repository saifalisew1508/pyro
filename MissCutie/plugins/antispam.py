...
from MissCutie import LOGGER, MESSAGE_DUMP, SUPPORT_GROUP, SUPPORT_STAFF
...
@Bot.on_message(command(["gban", "globalban"], sudo_cmd=True))
async def gban(c: Bot, m: Message):
...
async def ungban(c: Bot, m: Message):
    if len(m.text.split()) == 1:
