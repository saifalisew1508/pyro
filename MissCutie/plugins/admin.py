...
from MissCutie import DEV_USERS, LOGGER, OWNER_ID, SUPPORT_GROUP, SUPPORT_STAFF
from MissCutie.bot_class import Bot
from MissCutie.database.approve_db import Approve
from MissCutie.database.reporting_db import Reporting
from MissCutie.tr_engine import tlang
from MissCutie.utils.caching import TEMP_ADMIN_CACHE_BLOCK, admin_cache_reload
from MissCutie.utils.custom_filters import (
    DEV_LEVEL,
    admin_filter,
    chatonly,
    command,
...
@Bot.on_message(command("adminlist"))
@chatonly
...
@Bot.on_message(command("zombies") & owner_filter)
async def zombie_clean(c: Bot, m: Message):
    zombie = 0
...
@Bot.on_message(command("admincache"))
@chatonly
...
@Bot.on_message(filters.regex(r"^(?i)@admin(s)?") & filters.group)
...
@Bot.on_message(command("fullpromote") & promote_filter)
async def fullpromote_usr(c: Bot, m: Message):
...
@Bot.on_message(command("promote") & promote_filter)
async def promote_usr(c: Bot, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(tlang(m, "admin.promote.no_target"))
        return
...
@Bot.on_message(command("demote") & promote_filter)
async def demote_usr(c: Bot, m: Message):
    global ADMIN_CACHE
...
@Bot.on_message(command("invitelink"))
@chatonly
async def get_invitelink(c: Bot, m: Message):
...
@Bot.on_message(command("setgtitle") & admin_filter)
...
@Bot.on_message(command("setgdes") & admin_filter)
...
@Bot.on_message(command("title") & admin_filter)
async def set_user_title(c: Bot, m: Message):
    user = await m.chat.get_member(m.from_user.id)
    if (
...
@Bot.on_message(command("setgpic") & admin_filter)
